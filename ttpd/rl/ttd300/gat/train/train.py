...
class RunConfig:
    ...
    eval_n_lo: int = 5
    eval_n_hi: int = 22
    eval_sizes: tuple = (10, 20)       # graded sizes within [eval_n_lo, eval_n_hi]
    eval_fracs: tuple = (0.25, 0.50, 0.75, 1.00)
    eval_seed: int = 99
    resume: bool = True
...
def default_log(log: EpochLog) -> None:
    ...
    if log.eval is not None:
        ...
        if log.eval.per_frac:
            parts = [f"f{int(round(f * 100)):03d}={v:+.0f}"
                     for f, v in sorted(log.eval.per_frac.items())]
            eval_str += f" ED[{' '.join(parts)}]"
    print(
...
def run(
    ...
    *,
    ...
    if last_ckpt is not None:
        ...
        if run_cfg.init_from:
            ...
            print(f"[init] policy weights warm-started from {run_cfg.init_from} "
                  f"(fresh optimiser/schedule; {n_loaded} tensors loaded"
    ...
    eval_set = build_eval_set(
        ...
        sizes=[s for s in run_cfg.eval_sizes
               if run_cfg.eval_n_lo <= s <= run_cfg.eval_n_hi] or None,
        fracs=run_cfg.eval_fracs,
    ...
    def env_factory(n_inst: int) -> TTPDEnv:
        return TTPDEnv(a280_path=a280_path, n=n_inst,
                       ...
                       ED_frac_range=run_cfg.ed_frac_range)
    ...
    for stage_idx in range(train_state.stage_idx, len(cfg.curriculum)):
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            if eval_set is not None and (train_state.global_epoch + 1) % run_cfg.eval_every == 0:
                eval_result = evaluate(policy, eval_set=eval_set, deterministic=True)
