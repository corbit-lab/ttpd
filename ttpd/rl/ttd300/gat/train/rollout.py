...
class RolloutBuffer:
    ...
    mask_j: torch.Tensor
    feat_j: torch.Tensor            # (R, N_max, F) per-candidate pointer features
    feat_k: torch.Tensor            # (R, N_max+1, F), dummy slot zeroed
...
def collect_rollouts(
    ...
    for _ in range(n_instances):
        ...
        while active:
            ...
            for bi, i in enumerate(active):
                ...
                rec_feat_j.append(out["feat_j"][bi])
                rec_feat_k.append(out["feat_k"][bi])
    ...
    F_J = rec_feat_j[0].shape[1] if rec_feat_j else 0
    F_K = rec_feat_k[0].shape[1] if rec_feat_k else 0
    ...
    fj_pad = torch.zeros(R, N_max, F_J)
    fk_pad = torch.zeros(R, N_slots_max, F_K)
    for r_i in range(R):
        ...
        fj_pad[r_i, :n_rec] = torch.as_tensor(rec_feat_j[r_i])
        fk_pad[r_i, :n_rec] = torch.as_tensor(rec_feat_k[r_i][:n_rec])
        fk_pad[r_i, N_max] = torch.as_tensor(rec_feat_k[r_i][n_rec])  # dummy slot (zeros)
