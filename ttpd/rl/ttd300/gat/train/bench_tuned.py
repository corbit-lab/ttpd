...
from env.instance import full_instance, load_ttd300
...
from train.checkpoint import (
    ...
    promote_to_best, save_checkpoint, warm_start_policy,
)
...
class BenchTunedConfig:
    run_dir: str
    bench_files: list[str]            # TRAINING pool: the 5 base ttd300_n<N>_L*.txt paths
    n: int                            # full instance size of each file
    eval_files: list[str] | None = None  # PROMOTION grid: the _fXXX endurance files
    ...
    resume: bool = True
    ed_frac_range: tuple | None = (0.25, 1.00)
    ed_frac_power: float = 1.0
    synth_frac: float = 0.0
    synth_n_lo: int | None = None
    synth_n_hi: int | None = None
...
def _beam_eval(policy, eval_paths, eval_envs, eval_insts, bench_cfg: BenchTunedConfig,
               beam_width: int, beam_n_aug: int) -> dict:
    policy.eval()
    per_file = {}
    for path, env, inst in zip(eval_paths, eval_envs, eval_insts):
...
def run_bench_tuned(cfg: PPOConfig, bench_cfg: BenchTunedConfig) -> None:
    ...
    bench_data = [load_ttd300(p) for p in bench_cfg.bench_files]
    n_files = len(bench_cfg.bench_files)
    eval_paths = bench_cfg.eval_files if bench_cfg.eval_files else bench_cfg.bench_files
    eval_data = [load_ttd300(p) for p in eval_paths]
    eval_insts = [full_instance(d, scale_capacity=bench_cfg.scale_capacity)
                  for d in eval_data]
    eval_envs = [TTPDEnv(a280_data=d, n=bench_cfg.n, scale_capacity=bench_cfg.scale_capacity)
                 for d in eval_data]
    _log(bench_cfg, f"capacity check: scale_capacity={bench_cfg.scale_capacity}  "
                f"W(train files)={[round(float(d['params']['W'])) for d in bench_data]}")
    _log(bench_cfg, f"eval grid: {len(eval_paths)} instances  "
                f"ED={[None if i.ED is None else round(float(i.ED)) for i in eval_insts]}")
    ...
    if last_ckpt is not None:
        ...
        if bench_cfg.init_from:
            ...
            n_loaded, skipped = warm_start_policy(policy, payload["policy"])
            _log(bench_cfg, f"[init] warm-started policy from {bench_cfg.init_from} "
                        f"(fresh optimiser/schedule, lr={bench_cfg.lr:.1e}; {n_loaded} tensors"
                        + (f", {len(skipped)} skipped on shape mismatch: {skipped}" if skipped else "")
                        + ")")
        else:
    ...
    def env_factory(_n):
        if bench_cfg.synth_frac > 0.0 and rng.random() < bench_cfg.synth_frac:
            lo = bench_cfg.synth_n_lo if bench_cfg.synth_n_lo is not None else bench_cfg.n
            hi = bench_cfg.synth_n_hi if bench_cfg.synth_n_hi is not None else bench_cfg.n
            n_synth = int(rng.integers(lo, hi + 1)) if hi > lo else lo
            return TTPDEnv(a280_data=None, n=n_synth,
                           scale_capacity=bench_cfg.scale_capacity,
                           ED_frac_range=bench_cfg.ed_frac_range,
                           ED_frac_power=bench_cfg.ed_frac_power)
    ...
    if bench_cfg.init_from and train_state.global_epoch == 0 and bench_cfg.eval_every > 0:
        ev0 = _beam_eval(policy, eval_paths, eval_envs, eval_insts, bench_cfg,
    ...
    for gep in range(train_state.global_epoch, bench_cfg.total_epochs):
        ...
        if is_eval:
            ev = _beam_eval(policy, eval_paths, eval_envs, eval_insts, bench_cfg,
                            bench_cfg.eval_beam_width, bench_cfg.eval_beam_n_aug)
        ...
        if is_eval:
            if ev["mean"] > train_state.best_eval_return:
                ...
                evals_since_best += 1
                if (bench_cfg.early_stop_evals > 0
                        and evals_since_best >= bench_cfg.early_stop_evals):
                    _log(bench_cfg, f"[early-stop] {evals_since_best} consecutive evals "
                                f"without a promotion (best {train_state.best_eval_return:+.1f}); "
                                f"stopping at ep#{gep} of {bench_cfg.total_epochs}")
                    logger.write({"type": "early_stop", "global_epoch": gep,
...
def make_curriculum(bench_cfg: BenchTunedConfig, cfg: PPOConfig) -> None:
    """One flat stage spanning the whole run (all bench files are trained jointly
    every epoch inside run_bench_tuned). lr comes from bench_cfg.lr (fine-tune default)."""
