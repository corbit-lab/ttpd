...
class PPOStats:
    ...
    grad_norm: float = 0.0          # mean clipped grad norm
    n_updates: int = 0
...
class PPOTrainer:
    ...
    def update(self, buf: RolloutBuffer) -> PPOStats:
        ...
        feat_j = buf.feat_j.to(device)
        feat_k = buf.feat_k.to(device)
    ...
    @staticmethod
    def _normed_entropy(ent: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        # Entropy in units of "fraction of max": H / log(#valid actions). The raw
        # k/j-head entropy grows ~log(n), so a fixed coefficient pushes harder at
        # large n -- the documented divergence mode of the warm-started beasts.
        n_valid = mask.sum(dim=-1).clamp(min=2).float()
        return ent / n_valid.log()
    ...
    def _update_minibatch(self, *, feats_enc, node_mask_per_encoder,
        ...
        out = self.policy.evaluate_batch(
            ...
            feat_j=feat_j[idx], feat_k=feat_k[idx],
        )
        ...
        if getattr(cfg, "normalise_entropy", True):
            ent_term = (self._normed_entropy(hent["rejoin"], mask_rejoin[idx])
                        + self._normed_entropy(hent["z_drone"], mask_zd[idx])
                        + self._normed_entropy(hent["z_curr"], mask_zc[idx])
                        + self._normed_entropy(hent["k"], mask_k_ext[idx])
                        + self._normed_entropy(hent["j"], mask_j[idx]))
        else:
            ent_term = out["entropy"]
        entropy_loss = -ent_term.mean()
