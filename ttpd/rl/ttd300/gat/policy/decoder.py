...
class _PointerHead(nn.Module):
    ...
    def __init__(self, cfg: DecoderConfig, ctx_dim: int, feat_dim: int = 0):
        super().__init__()
        H = cfg.n_heads
        d = cfg.d_head
        D = cfg.d_model
        ...
        if feat_dim > 0:
            ...
            self.feat_mlp = nn.Sequential(nn.Linear(feat_dim, hidden), nn.GELU(), last)
        else:
            self.feat_mlp = None
    ...
    def forward(
        ...
        mask: torch.Tensor,
        feats: torch.Tensor | None = None,   # (B, M, feat_dim) per-candidate scalars
    ) -> torch.Tensor:
        B, M, D = h_nodes.shape
        H = self.H
        d = self.d_head
        ...
        if self.feat_mlp is not None:
            if feats is None:
                raise ValueError("pointer head built with feat_dim > 0 but no feats passed")
            u = u + self.feat_mlp(feats).squeeze(-1)
...
class StructuredDecoder(nn.Module):
    ...
    def __init__(self, cfg: DecoderConfig | None = None):
        ...
        self.head_k = _PointerHead(self.cfg, ctx_dim=D, feat_dim=self.cfg.feat_dim_k)
        ...
        self.head_j = _PointerHead(self.cfg, ctx_dim=2 * D, feat_dim=self.cfg.feat_dim_j)
    ...
    def head_k_forward(
        ...
        return self.head_k(ctx, h_with_dummy, mask_k_ext, feats=feat_k)
    ...
    def head_j_forward(
        ...
        return self.head_j(q_ctx, h_nodes, mask_j, feats=feat_j)
