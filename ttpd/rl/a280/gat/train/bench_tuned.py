...
class BenchTunedConfig:
    ...
    eval_beam_width: int = 16        
    eval_beam_n_aug: int = 2
    final_beam_width: int = 128       
    final_beam_n_aug: int = 8
    beam_stratify: bool = False       
    scale_capacity: bool = False
    resume: bool = True
    ed_frac_range: tuple | None = None
...
def _beam_eval(policy, bench_envs, bench_insts, bench_cfg: BenchTunedConfig,
    ...
    """Mean beam objective over the 5 bench instances. Returns per-file + mean."""
    policy.eval()
    per_file = {}
...
def run_bench_tuned(cfg: PPOConfig, bench_cfg: BenchTunedConfig) -> None:
    ...
    # round-robin env factory: collect_rollouts() pulls the next bench file on each call, so n_instances = n_files * reps covers every instance reps times/epoch.
    rr = {"i": 0}
