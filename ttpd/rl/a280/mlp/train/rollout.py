...
def collect_rollouts(
    ...
    for _ in range(n_instances):
        ...
        # close out the instance group (episode order = POMO index order; each
        # episode's steps are contiguous in its own index list, not in the flat
        # arrays; ep_ranges generalises to explicit index lists below)
        for p in range(pomo_size):
