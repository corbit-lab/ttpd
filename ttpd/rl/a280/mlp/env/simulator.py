...
from env.instance import TTPDInstance, load_a280, sample_instance
...
class TTPDEnv:
    ...
    def mask_z_curr_after(self, rejoin: int, z_drone: int) -> np.ndarray:
        """z_curr feasibility *given* a pending rejoin+z_drone this step. step()
        validates z_curr against the post-rejoin truck load, so when the drone
        lands a collected item (rejoin=1, z_drone=1) its weight must be added
        first. Matters only when the truck is near capacity (large n); at small n
        capacity is loose and this equals mask_z_curr (no behaviour change)."""
        inst = self.inst
        W_eff = self.W_t
    ...
    def masks_for_launch(self, rejoined: bool) -> dict:
        # launch-target mask given whether a rejoin already happened this step
        # (which frees the drone to be relaunched).
        inst = self.inst
    ...
    def mask_j_after_launch(self, k: int, rejoined: bool) -> np.ndarray:
        # truck-destination mask given the launch decision. Excluded: the new launch
        # target k, and the current in-flight target (still committed if no rejoin,
        # served-on-landing if rejoined; either way not a truck destination). The
        # sink opens up once no other customers remain. Mirrors step()'s m_j check.
        inst = self.inst
