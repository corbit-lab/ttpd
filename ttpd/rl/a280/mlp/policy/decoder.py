...
class DecoderConfig:
    d_model: int = 128
    n_heads: int = 8
    # [W_norm, tau_norm, U_frac, R_norm, drone_norm, drone_wait_norm, drone_elapsed_norm]
    scalar_dim: int = 7
...
class StructuredDecoder(nn.Module):
    ...
    def __init__(self, cfg: DecoderConfig | None = None):
        ...
        # binary heads. rejoin & z_drone are conditioned on the in-flight drone target h_D; z_curr on the current node h_c.
        def _binary_head():
