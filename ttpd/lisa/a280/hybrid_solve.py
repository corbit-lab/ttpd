...
from core import (Problem, evaluate, check_solution,    
...
def load_bench(n: int, instance: int):
...
def run(args):
    ...
    for n in args.sizes:
        for inst_no in args.instances:
            P, inst = load_bench(n, inst_no)
            sol = gat_decode_solution(policy, inst, mode=args.decode,
                                      beam_width=args.beam_width, n_aug=args.n_aug)
            label = f"bench_{n}_{inst_no}"
            if sol is None:
                print(f"{label}: GAT decode did not terminate -- skipped", flush=True)
                continue
            ...
            if err is not None or abs(gat_obj - gat_env_obj) > CONS_EPS:
                print(f"{label}: GAT solution inconsistent ({err}) -- skipped", flush=True)
                continue
            print(f"{label}: GAT beam decoded (obj {gat_obj:.1f}), SA queued", flush=True)
    ...
    hdr = (f"{'instance':<13} {'GAT':>11} {'SA_scratch':>11} {'SA_warm':>11} "
    ...
    for i, (label, P, gat_obj, _init) in enumerate(insts):
        ...
        print(f"{label:<13} {gat_obj:11.1f} {sa_o:11.1f} "
    ...
    if agg["sa"]:
        ...
        print("-" * len(hdr))
        print(f"{'MEAN':<13} {gat_m:11.1f} {sa_m:11.1f} {warm_m:11.1f} "
...
def main():
    ...
    ap.add_argument("--weights", type=str, required=True, help="BC or BC->PPO checkpoint")
    ap.add_argument("--sizes", type=int, nargs="+", default=[5, 10])
    ap.add_argument("--instances", type=int, nargs="+", default=[1, 2, 3, 4, 5])
