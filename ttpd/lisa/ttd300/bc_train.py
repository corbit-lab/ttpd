...
from ttd300_io import GAT, TEMPLATE                       
...
def instance_from_dict(d: dict) -> TTPDInstance:
    return TTPDInstance(
        ...
        ED=None if d.get("ED") is None else float(d["ED"]),
    )
...
def _tolerant_load(policy, path, device):
    payload = torch.load(ensure_local(path), map_location=device, weights_only=False)
    ...
    policy.load_state_dict(usable, strict=False)
    print(f"[bc] warm-started from {path}: {len(usable)}/{len(own)} tensors"
...
def main():
    ...
    ap.add_argument("--init-from", type=str, default=None,
                    help="warm-start policy weights from a prior BC checkpoint "
                         "(the size-progressive chain: n10 -> n20 -> ...; "
                         "shape-mismatched tensors are skipped)")
    ...
    if args.init_from:
        _tolerant_load(policy, args.init_from, device)
