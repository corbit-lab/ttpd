...
def load_ckpt_adapted(checkpoint: str, device: str) -> AttentionPolicy:
    ...
    D = dec_cfg.d_model
    ckpt_scalars = ckpt_in - 3 * D  # ctx = [h_graph, h_c, h_meanU, scalars]
...
def main() -> None:
    ...
    p.add_argument("--bench-dir", required=True,
                   help="directory containing bench_<n>_<inst>.txt")
    p.add_argument("--ckpt-dir", default=hub_weights("a280", "gat", "benchmark-tuned"),
                   help="dir with gat_n<size>/checkpoints/best.pt")
    p.add_argument("--ckpt-map", default=None,
                   help="explicit n:path[,n:path...] checkpoint map "
                        "(overrides --ckpt-dir; use to bypass broken symlinks)")
    p.add_argument("--sizes", default="5,10,15,20")
    p.add_argument("--inst", type=int, default=5, help="instance number")
    ...
    for n in sizes:
        ...
        if err is not None:
            ...
            continue
        rows.append({"method": "GAT", "n": n, "instance": f"bench_{n}_{args.inst}",
