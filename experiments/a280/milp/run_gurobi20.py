...
def main():
    ap = argparse.ArgumentParser(description="batch TTP-D MILP runner, n=20.")
    ap.add_argument("--seconds", type=float, default=15.0,
                    help="Time limit per instance, seconds (default 15).")
    ap.add_argument("--seed-list", default="5",
                    help="Comma-separated instance ids to run (default 5 = "
                         "bench_20_5, the single benchmark instance).")
    ap.add_argument("--threads", type=int, default=0,
                    help="Gurobi threads per solve. 0 (default) = ALL cores on "
                         "this machine (os.cpu_count()).")
    ap.add_argument("--mip-gap", type=float, default=0.0001,
                    help="Gurobi MIPGap (default 0.0001 = 0.01%%).")
    ap.add_argument("--out", default=os.path.join(HERE, "results_gurobi20.csv"))
    ap.add_argument("--logfile", default=os.path.join(HERE, "run_gurobi20.log"))
    ...
    threads = cli.threads if cli.threads > 0 else (os.cpu_count() or 1)
    os.environ["GRB_THREADS"] = str(threads)
    ...
    log(f"GUROBI n={N_CITIES}  seeds={seeds}  limit={cli.seconds}s/inst  "
        f"threads={threads} (all cores)  mip_gap={cli.mip_gap}")
    log(f"out={out_path}")
    log("=" * 70)
