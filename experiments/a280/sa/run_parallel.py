...
from run_sa import SIZES, INSTANCES, SEEDS, TIME_BUDGET  # noqa: E402
...
def main():
    ap = argparse.ArgumentParser(description="Parallel SA sweep launcher.")
    ap.add_argument("--shards", type=int, default=4,
                    help="number of parallel processes = number of vCPUs "
                         "(default 4, matching e2-standard-4)")
    ...
    print("=" * 72)
    print(f"PARALLEL SA SWEEP  shards={cli.shards}  "
    ...
    shard_csvs = []
    for k, sh in enumerate(shards):
        csv_path = os.path.join(HERE, f"results_sa_shard{k}.csv")
        log_path = os.path.join(HERE, f"run_sa_shard{k}.log")
        ...
        for n in sorted(sh):
            ...
            cmds.append(
                f'{sys.executable} {os.path.join(HERE, "run_sa.py")} '
