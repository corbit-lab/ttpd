...
SA_ALPHA = 0.97                      # geometric cooling factor (sa.py default)
...
MILP_REFS: dict = {}        # (n, inst_no) -> MILP optimal objective
...
def main():
    ap = argparse.ArgumentParser(
        description="Sequential SA benchmark runner (flat, batch-ready).")
    ...
    ap.add_argument("--out", default=os.path.join(HERE, "results_sa.csv"),
                    help="results CSV (appended, fsync'd per row)")
    ap.add_argument("--logfile", default=os.path.join(HERE, "run_sa.log"))
    cli = ap.parse_args()
    ...
    log("=" * 78)
    log(f"SA BENCHMARK RUNNER  pid={os.getpid()}  host={os.uname().nodename}")
    log(f"sizes={sizes}  instances={instances}  seeds={seeds}")
    log(f"time budgets={ {n: TIME_BUDGET[n] for n in sizes if n in TIME_BUDGET} }")
    log(f"loaded {len(MILP_REFS)} per-instance MILP refs for gap_% in CSV "
        f"(gap is NOT printed to the logs)")
    log(f"out={out_path}")
    log("=" * 78)
    ...
    for n in sizes:
        ...
        for inst_no in instances:
            ...
            for seed in seeds:
                ...
                # NOTE: gap deliberately omitted from the log line (misleading
                # mid-sweep); it is still in the CSV's gap_% column.
                log(f"  [{done}/{grand_total}] n={n} inst={inst_no} "
                    f"seed={seed}  obj={row.get('objective')}  "
                    f"t={row.get('time_s')}s  "
                    f"ttb={row.get('time_to_best_s')}s  "
                    f"reheats={row.get('reheats')}  "
