...
class MProblem:
    ...
    def __init__(self, path):
        ...
        # node ordering: file index order, depot (id 1) first already.
        ids = sorted(coords)
        self.node_ids = ids
        self.n = len(ids) - 1                       # customers (depot excluded)
        self.src, self.snk = 0, self.n + 1
        nn = self.n + 2
        # slot 0 = depot(src), 1..n = customers, n+1 = depot(sink)
        ...
        self.R = float(params.get("R", 50.0))
        self.ED = params.get("ED")                  # drone endurance (ttd300)
        assert self.ED is not None, f"no DRONE ENDURANCE header in {path}"
    ...
    def _parse(path):
        ...
        with open(path) as f:
            for line in f:
                ...
                elif s.startswith("DRONE ENDURANCE"):
                    params["ED"] = float(s.split(":")[1])
...
def ws_ps(P, node, mask):
    """Selected (weight, profit) sums at a node given its item bitmask."""
    w = p = 0.0
    its = P.items[node]
    m = mask
    i = 0
...
def evaluate_m(P, truck, sorties, sel):
    """Env-exact multi-item objective; None if infeasible
    (anchors/capacity/endurance)."""
    ...
    if sorties:
        ...
        for idx, (L, D, J) in enumerate(sord):
            ...
            if pL is None or pJ is None or pL >= pJ or pL < last_end:
                return None
            # Drone endurance: round-trip launch->target->rejoin must fit.
...
def solution_stats(P, truck, sorties, sel):
    ...
    return {
        ...
        "items_dist": dict(sorted(_hist(per_city.values()).items())),
...
def run_mi(path: str, budget: float, seed: int = SEED) -> dict:

    P = MProblem(path)
    truck, sorties, sel, obj, evals, wall = sa_multi(P, budget, seed=seed)
    for (L, D, J) in sorties:
        reach = P.dist[L][D] + P.dist[D][J]
        if reach > P.ED + 1e-6:
            raise RuntimeError(
                f"SORTIE EXCEEDS ED {reach:.2f} > {P.ED} ({L},{D},{J})")
    st = solution_stats(P, truck, sorties, sel)
    return {
        "objective": round(obj, 4),
        "W_capacity": round(P.Wcap, 1), "W_actual": round(P.W_actual, 1),
        "ED": round(float(P.ED), 4), "R": P.R,
        "evals": evals, "time_s": round(wall, 2),
        **st,
    }
...
def main():
    ...
    path = E.bench_path(cli.n, cli.layout, cli.frac)
    budget = cli.budget if cli.budget is not None else E.TIME_BUDGET[cli.n]

    base = E.run_config(path, budget, seed=cli.seed)
    r = run_mi(path, budget, seed=cli.seed)
    print(f"n={cli.n} L{cli.layout} f={cli.frac}  "
          f"MI_obj={r['objective']}  SI_obj={base['objective']}  "
          f"delta={round(r['objective'] - base['objective'], 4)}  "
          f"cities={r['cities_collected']} items={r['total_items']} "
          f"multi={r['multi_pick_cities']} max/city={r['max_items_one_city']} "
          f"cap={r['cap_util_%']}%")
